...
/**
 * Search for events using Tavily API with retry logic
 */
async function searchEventsWithTavily(location: string, dateRange: string, retries: number = 2): Promise<any> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    if (attempt > 0) {
      console.log(`[EventsAgent] Retry attempt ${attempt}/${retries}`)
    ...
    try {
      ...
      console.log(`[EventsAgent] Successfully fetched ${result.results?.length || 0} results`)
      return result
      ...
      if (fetchError instanceof Error && fetchError.name === 'AbortError') {
        console.error(`[EventsAgent] Tavily API timeout after 20s (attempt ${attempt + 1}/${retries + 1})`)
      } else {
        console.error(`[EventsAgent] Search error (attempt ${attempt + 1}/${retries + 1}):`, fetchError)
      }
  ...
  // All retries failed
  console.warn('[EventsAgent] All retry attempts failed, returning empty results')
  return { results: [] }
}

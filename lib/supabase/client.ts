// Simple Supabase client using fetch API for v0 compatibility
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          ...
          Authorization: `Bearer ${getStoredToken() || SUPABASE_ANON_KEY}`,
        },
      })
...
class SupabaseInsertBuilder<T = any> {
  private tableName: string
  private data: any
...
class SupabaseUpdateBuilder<T = any> {
  private tableName: string
  private data: any
  private filters: string[] = []
  private returnData = false
...
class SupabaseDeleteBuilder<T = any> {
  private tableName: string
  private filters: string[] = []
...
class SupabaseUpsertBuilder<T = any> {
  private tableName: string
  private data: any
  private returnData = false
...
class SupabaseTableBuilder {
  ...
  upsert(data: any) {
    return new SupabaseUpsertBuilder(this.tableName, data)
  }
}
...
const auth = {
  ...
  async signUp({ email, password, options }: { email: string; password: string; options?: any }): Promise<AuthResult> {
    try {
  ...
  async getSession(): Promise<{ data: { session: any } | null; error: null }> {
    const token = getStoredToken()
  ...
  async getUser(): Promise<{ data: { user: any } | null; error: null }> {
    const user = getStoredUser()
...
const supabaseClient = {
  from(table: string) {
    return new SupabaseTableBuilder(table)
  },
  auth,
}
